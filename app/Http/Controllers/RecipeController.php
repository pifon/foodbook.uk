...
class RecipeController extends Controller
{
    ...
    /**
     * Build [ slug => label ] from a list of cuisine items (from flatten or raw attributes).
    ...
    /**
     * Build JSON:API payload to match API spec: attributes (title, description,
     * prep-time-minutes, cook-time-minutes, difficulty, serves),
    ...
    private function recipeAttributes(Request $request): array
    {
        $attrs = [
            ...
            'prep-time-minutes' => $request->filled('prep_time_minutes')
                ? (int) $request->input('prep_time_minutes') : null,
            'cook-time-minutes' => $request->filled('cook_time_minutes')
                ? (int) $request->input('cook_time_minutes') : null,
            'difficulty' => $request->input('difficulty'),
            'serves' => $request->filled('serves')
                ? (int) $request->input('serves') : null,
        ];
    ...
    private function isPreparationMethodNotSupported(Response $response): bool
    {
        ...
        $notSupported = str_contains($body, 'not supported');
        $supportedMethods = str_contains($body, 'supported methods');
        return str_contains($body, 'method') && ($notSupported || $supportedMethods);
    }
